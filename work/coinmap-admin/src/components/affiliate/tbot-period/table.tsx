...
const GuestsTable: React.FC<GuestsTableProps> = (props) => {
  ...
  const columns: ColumnsType<BotPeriod> = useMemo(
    () => [
      {
        title: 'Name',
      ...
      },
      {
        ...
        align: 'right',
        width: '10%',
        render: (value: BotPeriod['created_at']) => formatDate(Number(value)),
      },
      {
        title: '',
        dataIndex: 'action',
        ...
        align: 'center',
        width: '1%',
        render: (_: BotPeriod, record: BotPeriod) => (
          <Tooltip title="Update BOT">
            <Button
              onClick={() =>
                router.push(`${ROUTERS.AFFILIATE_TBOT_PERIOD}/${record.id}`)
              }
              icon={
