...
const renderTabContent = (key: string) => {
  return [1, 2, 3].map((value) => (
    <Fragment key={value}>
      <Title level={4}>Feature {value}</Title>
      <Row gutter={16}>
        <Col span={24} lg={8}>
          <Form.Item
            name={[key, 'features', `feature_${value}`, 'title']}
        ...
        <Col span={24} lg={16}>
          <Form.Item
            name={[key, 'features', `feature_${value}`, 'description']}
            label={<Text strong>Description</Text>}
            rules={[{ required: true, message: 'Field is required' }]}
          >
            <TextArea size="large" placeholder="Enter description" allowClear />
          </Form.Item>
        </Col>
      </Row>
    </Fragment>
...
const Features = ({ showResetDefault, handleResetDefault }: Props) => {
