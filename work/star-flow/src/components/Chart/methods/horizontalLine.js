...
import { getShowableHorizontalLines } from '@/utils/horizontalLine';
import { intervalToSeconds } from '@/utils/calculator';

export function horizontalXLine_onComplete(items, { fullData }) {
  const { chartId, dispatch, draws, interval } = this.props;
  const drawData = draws?.[DRAW_TOOLS.horizontal_line.type]?.data || {};
  ...
  this.setState({
    draws: {
      ...this.state.draws,
      [DRAW_TOOLS.horizontal_line.type]: items,
    },
  });

  dispatch(
    actEndDraw(DRAW_TOOLS.horizontal_line.type, chartId, {
      ...drawData,
      items,
    })
  );
...
export function horizontalLine_getShowables(
  timeIndexMap,
  interval,
  lastCandle
) {
  const { draws } = this.props;
  const drawType = DRAW_TOOLS.horizontal_line.type;
  const { draws: { [drawType]: currentItems = [] } = {} } = this.state || {};
  const horizontalLines = getShowableHorizontalLines({
    timeIndexMap,
