...
import { getShowableShortPosition } from '@/utils/shortPosition';
import { intervalToSeconds } from '@/utils/calculator';

export function shortPosition_onComplete(items, { fullData }) {
  const { chartId, dispatch, draws, interval } = this.props;
  const drawData = draws?.[DRAW_TOOLS.short_position.type]?.data || {};
  ...
  this.setState({
    draws: {
      ...this.state.draws,
      [DRAW_TOOLS.short_position.type]: items,
    },
  });

  dispatch(
    actEndDraw(DRAW_TOOLS.short_position.type, chartId, {
      ...drawData,
      items,
    })
  );
...
export function shortPosition_getShowables(
  timeIndexMap,
  interval,
  lastCandle,
  firstCandle
  ...
  const drawType = DRAW_TOOLS.short_position.type;
  const { draws: { [drawType]: currentItems = [] } = {} } = this.state || {};
  const shortPositions = getShowableShortPosition({
    timeIndexMap,
