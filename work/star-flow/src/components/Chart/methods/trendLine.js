...
import { getShowableTrendLines } from '@/utils/trendLine';
import { intervalToSeconds } from '@/utils/calculator';

export function trendLine_onComplete(items, { fullData }) {
  ...
  this.setState({
    draws: {
      ...this.state.draws,
      [DRAW_TOOLS.trend_line.type]: items,
    },
  });

  dispatch(
    actEndDraw(DRAW_TOOLS.trend_line.type, chartId, {
      ...drawData,
      items,
    })
  );
...
export function trendLine_getShowables(
  timeIndexMap,
  interval,
  lastCandle,
  firstCandle
  ...
  const drawType = DRAW_TOOLS.trend_line.type;
  const { draws: { [drawType]: currentItems = [] } = {} } = this.state || {};
  const trendLines = getShowableTrendLines({
    timeIndexMap,
