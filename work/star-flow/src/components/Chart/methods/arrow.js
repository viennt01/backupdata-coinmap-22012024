...
import { getShowableArrow } from '@/utils/arrow';
import { intervalToSeconds } from '@/utils/calculator';

export function arrow_onComplete(items, { fullData }) {
  ...
  this.setState({
    draws: {
      ...this.state.draws,
      [DRAW_TOOLS.arrow.type]: items,
    },
  });

  dispatch(
    actEndDraw(DRAW_TOOLS.arrow.type, chartId, {
      ...drawData,
      items,
    })
  );
...
export function arrow_getShowables(
  timeIndexMap,
  interval,
  lastCandle,
  firstCandle
  ...
  const drawType = DRAW_TOOLS.arrow.type;
  const { draws: { [drawType]: currentItems = [] } = {} } = this.state || {};
  const arrow = getShowableArrow({
    timeIndexMap,
