...
export function Carousel(props) {
  ...
  const [height, setHeight] = useState('489px');
    ...
    if (slideTotal > 1) {
      ...
      slide.forEach((slid, index) => {
        ...
        if (slid.class.includes('active')) {
          if (index < slideCurrentLoc) {
            slid.class = 'slider-single preactivede';
          } else {
            slid.class = 'slider-single proactivede'; // proac is next image
          }
        }
      });
  ...
  return (
    <div className="react-3d-carousel" style={{ height }} {...handlers}>
          ...
          <div className="slider-content">
            {slides.map((slider, index) => (
              <div className={slider.class} key={index}>
                <div className={sliderClass('left')} onClick={slideLeft}></div>
                <div
      ...
      <ul
        ...
        className="slick-dots relative pt-[20px]"
      >
        {slides.length > 0 &&
          slides.map((_, i) => {
            const className = slideCurrent === i ? 'slick-active' : '';
            return (
              <li
                key={i}
                onClick={() => {
