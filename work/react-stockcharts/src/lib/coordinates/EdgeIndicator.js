...
EdgeIndicator.propTypes = {
  ...
  xCountDown: PropTypes.number,
  yCountDown: PropTypes.number,
  fontStyle: PropTypes.string,
  lineStrokeDasharray: PropTypes.oneOf(strokeDashTypes),
  marginY: PropTypes.number, // margin y
...
function getEdge(props, moreProps, item) {
  ...
  const { yAccessor, fill, textFill, rectHeight, rectWidth, arrowWidth, xCountDown, yCountDown, fontStyle } =
    props;
  ...
  const x1 = Math.round(xScale(xValue)),
    y1 = Math.round(yScale(yValue)) + marginY;
