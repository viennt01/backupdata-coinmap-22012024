...
import { getBackgroundCanvas, getAxisCanvas } from "../GenericComponent";
...
class Axis extends Component {
	constructor(props) {
		...
		this.drawTickLabelsOnCanvas = this.drawTickLabelsOnCanvas.bind(this);
		this.drawTicksOnCanvas = this.drawTicksOnCanvas.bind(this);
	...
	drawTickLabelsOnCanvas(ctx, moreProps) {
		const { showDomain, showTicks, transform, range, getScale, showBackground, drawTickLabelsProps } = this.props;

		ctx.save();
		ctx.translate(transform[0], transform[1]);

		if (showBackground) drawAxisBackground(ctx, this.props, range);
		if (showDomain) drawAxisLine(ctx, this.props, range);
		if (showTicks) {
			const tickProps = tickHelper(this.props, getScale(moreProps));
			if (drawTickLabelsProps) {
				drawTickLabelsProps(ctx, tickProps);
			} else {
	...
	drawTicksOnCanvas(ctx, moreProps) {
		const { showTicks, transform, getScale, drawTicksProps } = this.props;
	...
	render() {
		...
		const { transform, getMouseDelta, edgeClip, enableWheelX, enableWheelY, zoomMultiplier } = this.props;
		...
		return <g transform={`translate(${transform[0]}, ${transform[1]})`}>
			...
			<GenericChartComponent
				...
				clip={false}
				edgeClip={edgeClip}
				canvasDraw={this.drawTickLabelsOnCanvas}
				drawOn={["pan"]}
			/>
			<GenericChartComponent
				canvasToDraw={getBackgroundCanvas}
				clip={false}
				edgeClip={edgeClip}
				canvasDraw={this.drawTicksOnCanvas}
				drawOn={["pan"]}
			/>
		</g>;
	}
...
Axis.propTypes = {
	...
	drawTicksProps: PropTypes.func,
	drawTickLabelsProps: PropTypes.func,
	enableWheelX: PropTypes.bool,
	enableWheelY: PropTypes.bool,
	zoomMultiplier: PropTypes.number,
	width: PropTypes.number, // using for yAxis
	height: PropTypes.number, // using for xAxis
};
...
function drawAxisBackground(ctx, props, range) {
	// props = { ...AxisLine.defaultProps, ...props };

	const { orient, outerTickSize, fillBackground, height, width } = props;
	...
	if (xAxis) {
		// x, y, width, height
		ctx.rect(first(range), sign * outerTickSize, last(range), height);
	} else {
		ctx.rect(first(range), sign * outerTickSize, width, last(range));
	}
	ctx.lineWidth = 0;
	ctx.strokeWidth = 0;
