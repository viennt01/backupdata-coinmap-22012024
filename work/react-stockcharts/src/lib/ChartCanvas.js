...
import CanvasContainer, { CANVAS_IDS } from "./CanvasContainer";
...
// Set y range base on options
function setYRange(scale, height, padding, flipYScale) {
...
function getCursorStyle() {
	...
	.react-stockcharts-nesw-resize-cursor {
		cursor: nesw-resize;
	}
	.react-stockcharts-nwse-resize-cursor {
		cursor: nwse-resize;
	}
	.react-stockcharts-grab-cursor {
		cursor: -webkit-grab;
		cursor: grab;
	}`;
...
class ChartCanvas extends Component {
	constructor() {
		...
		this.amIOnTopHovering = this.amIOnTopHovering.bind(this);
	...
	clearListCanvas(canvasIds) {
		const canvases = this.getCanvasContexts();
		if (!canvases) return;
		canvasIds.forEach((id) => {
			if (canvases[id]) {
				clearCanvas([canvases[id]], this.props.ratio);
			}
		});
	}
	clearAllCanvas() {
		const canvases = this.getCanvasContexts();
		if (canvases && canvases[CANVAS_IDS.AXES]) {
			clearCanvas(Object.values(canvases), this.props.ratio);
		}
	}
	subscribe(id, rest) {
	...
	updateGroupId(suscriberId, groupId) {
		const index = this.subscriptions.findIndex((each) => each.id === suscriberId);
		if (index !== -1) {
			this.subscriptions[index].groupId = { ...groupId };
		}
	}
	getAllPanConditions() {
	...
	amIOnTopHovering(id) {
		for (let i = this.subscriptions.length - 1; i >= 0; i--) {
			const { id: suscriberId, enablePropagationHover, getMoreProps } = this.subscriptions[i];
			if (enablePropagationHover) continue;
			if (suscriberId === id) return true;
			if (getMoreProps && getMoreProps().hovering) return false;
		}
		return false;
	}
	...
	handlePinchZoom(initialPinch, finalPinch, e) {
		if (!this.waitingForPinchZoomAnimationFrame) {
			...
			requestCustomAnimationFrame(() => {
				this.clearListCanvas([CANVAS_IDS.AXES, CANVAS_IDS.MOUSE_COORD]);
	...
	handleZoom(zoomDirection, mouseXY, e) {
		...
		const { zoomMultiplierX, zoomMultiplierY, zoomAnchor, enableZoomY } = this.props;
		const { fullData } = this;
		...
		const c = zoomDirection > 0 ? 1 * zoomMultiplierX : 1 / zoomMultiplierX;
		...
		// Set scale Y on zoom
		const isXScaleStopZoom = xScale(2) === initialXScale(2);
		if (enableZoomY && !isXScaleStopZoom) {
			// disable zoom in when plot data has only 3 items
			if (zoomDirection < 0 && (!Array.isArray(initialPlotData) || initialPlotData.length <= 3)) {
				return;
			}

			chartConfig.forEach((chart) => {
				if (!currentCharts.includes(chart.id)) {
					return;
				}
				...
				const diffRatio = zoomDirection > 0 ? 1 * zoomMultiplierY : 1 / zoomMultiplierY;
				const newDomain = initYScale.range().map(yEdge => initYScale.invert(mouseY + (yEdge - mouseY) * diffRatio));
				const newYScale = setYRange(
					initYScale.copy().domain(newDomain), height, padding, flipYScale
				);
		...
		const newChartConfig = chartConfig.map((c => ({
			...c,
			yPanEnabled: enableZoomY && c.enableZoomY, // turn off pan y axis when user scroll
		})));
		...
		// queue to reset state
		if (!enableZoomY) {
			if (isDefined(this.zoomTimeout)) {
			...
			this.zoomTimeout = setTimeout(() => {
				const newChartConfig = chartConfig
					.map(each => {
						if (isNotDefined(currentCharts[0]) || each.id === currentCharts[0]) {
							return {
								...each,
								yPan: true,
								yPanEnabled: true,
	...
	yAxisZoom(chartId, newDomain) {
		 // 2 case: zoom scroll (yPanEnabled: false)  and drag y (yPanEnabled: true);
		...
		this.setState({
			...
			this.triggerEvent("zoomY", this.state, null);
		});
	}
	triggerEvent(type, props, e) {
		for (let i = this.subscriptions.length - 1; i >= 0; i--) {
			...
			this.subscriptions[i].listener(type, props, state, e);
		}
	}
	draw(props) {
		// sort subscriptions based on groupIndex and itemIndex
		this.subscriptions.sort((a, b) => {
			const aId = a.groupId?.id;
			const bId = b.groupId?.id;
			const aItemIndex = a.groupId?.itemIndex;
			const bItemIndex = b.groupId?.itemIndex;
			const aGroupIndex = a.groupId?.groupIndex;
			const bGroupIndex = b.groupId?.groupIndex;
			if (aId !== bId && aGroupIndex < bGroupIndex) return -1;
			if (aId !== bId && aGroupIndex > bGroupIndex) return 1;
			if (aId === bId && aItemIndex < bItemIndex) return -1;
			if (aId === bId && aItemIndex > bItemIndex) return 1;
			return 0;
		});
	...
	redrawCanvas(canvasId) {
		this.clearListCanvas([canvasId]);
		this.subscriptions.forEach(each => {
			if (isDefined(each.draw))
				each.redrawCanvas(canvasId);
		});
	}
	...
	handlePan(mousePosition, panStartXScale, dxdy, chartsToPan, e) {
		if (!this.waitingForPanAnimationFrame) {
			...
			requestCustomAnimationFrame(() => {
				this.clearListCanvas([CANVAS_IDS.AXES, CANVAS_IDS.MOUSE_COORD, CANVAS_IDS.DRAW, CANVAS_IDS.MOUSE_MARKER, CANVAS_IDS.BACKGROUND]);
	...
	handleMouseMove(mouseXY, inputType, e) {
		if (!this.waitingForMouseMoveAnimationFrame) {
			...
			requestCustomAnimationFrame(() => {
				this.clearListCanvas([CANVAS_IDS.MOUSE_COORD, CANVAS_IDS.DRAW, CANVAS_IDS.MOUSE_MARKER]);
	...
	handleDrag({ startPos, mouseXY }, e) {
		...
		requestCustomAnimationFrame(() => {
			this.clearListCanvas([CANVAS_IDS.MOUSE_COORD, CANVAS_IDS.MOUSE_MARKER]);
	...
	handleClick(mousePosition, e) {
		...
		requestCustomAnimationFrame(() => {
			this.clearListCanvas([CANVAS_IDS.MOUSE_COORD, CANVAS_IDS.DRAW]);
	...
	getChildContext() {
		...
		return {
			...
			amIOnTopHovering: this.amIOnTopHovering,
	...
	UNSAFE_componentWillReceiveProps(nextProps) {
		this.fps = typeof this.props.fps === 'number' ? this.props.fps : Infinity;
	...
	render() {
		...
		const { useCrossHairStyleCursor, onSelect, wheelPan } = this.props;
		...
		return (
			<div style={{ position: "relative", width, height }} tabIndex={0} className={className} onClick={onSelect}>
...
ChartCanvas.propTypes = {
	...
	zoomMultiplierX: PropTypes.number,
	zoomMultiplierY: PropTypes.number,
...
ChartCanvas.childContextTypes = {
	...
	amIOnTopHovering: PropTypes.func,
	redraw: PropTypes.func,
	redrawCanvas: PropTypes.func,
	subscribe: PropTypes.func,
	unsubscribe: PropTypes.func,
	updateGroupId: PropTypes.func,
