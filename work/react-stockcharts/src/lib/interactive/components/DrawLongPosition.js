...
class DrawLongPosition extends Component {
  hover1 = false;
  hover2 = false;
  ...
  drawOnCanvas = (ctx, moreProps) => {
    ...
    const stop = yEntryStart - yStopLoss;
    const stopP = (stop / yEntryStart) * 100;
    const target = yTakeProfit - yEntryStart;
    ...
    if (selected || alwaysShowStats) {
      ...
      if (xPxTakeProfit) {
        ...
        ctx.roundRect(
          ...
          yPxTakeProfit - textSize - textSize,
          width + textSize * 2,
          textSize * 2,
          4
        );
...
export default DrawLongPosition;
