...
class DrawOnePointLine extends Component {
  isHover = (moreProps) => {
    const { tolerance, onHover, getHoverInteractive } = this.props;

    if (isDefined(onHover)) {
      const { x1Value, y1Value, type } = this.props;
...
function helper(props, moreProps) {
  const { x1Value, y1Value, type } = props;
...
export function generateLine({ type, start, end, xScale, yScale }) {
  ...
  switch (type) {
    ...
    case "horizontalRay":
      return getHorizontalRayCoordinates({
        type,
        start,
        end,
        xScale,
    ...
    case "horizontalXline":
      return getHorizontalXLineCoordinates({
        type,
        start,
        end,
        xScale,
...
function getHorizontalRayCoordinates({ start, end, xScale, yScale, m, b }) {
  const [xBegin, xFinish] = xScale.domain();
  const [yBegin, yFinish] = yScale.domain();
  ...
  if (end[1] === start[1]) {
    return {
      x1: start[0],
      y1,
      x2: end[0] > start[0] ? xBegin : xFinish,
      y2: y1,
    };
  }

  const y2 = end[1] > start[1] ? yFinish : yBegin;
...
function getHorizontalXLineCoordinates({ start, end, xScale, yScale, m, b }) {
  const [xBegin, xFinish] = xScale.domain();
  const [yBegin, yFinish] = yScale.domain();
  ...
  const [y1, y2] = end[1] > start[1] ? [yBegin, yFinish] : [yFinish, yBegin];
...
export default DrawOnePointLine;
