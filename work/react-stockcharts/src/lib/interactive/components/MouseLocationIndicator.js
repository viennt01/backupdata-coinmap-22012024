...
class MouseLocationIndicator extends Component {
	...
	handleDbClick = (moreProps, e) => {
		const pos = this.xy(moreProps, e);
		if (isDefined(pos)) {
			const { xValue, yValue, x, y } = pos;
			this.mutableState = { x, y };
			this.props.onDoubleClick([xValue, yValue], moreProps, e);
		}
	}
	xy(moreProps, e) {
	...
	drawOnCanvas(ctx, moreProps) {
		...
		const { show , showCircle } = moreProps;
		if (enabled && show && isDefined(x) && showCircle) {
	...
	renderSVG(moreProps) {
		...
		return enabled && show && isDefined(x) && showCircle
			? <circle
				cx={x}
				cy={y}
				r={r}
