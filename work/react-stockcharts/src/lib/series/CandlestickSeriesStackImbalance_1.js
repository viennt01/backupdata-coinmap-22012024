...
class CandlestickSeriesStackImbalance extends Component {
	constructor(props) {
		super(props);
	...
	*/
	...
	for (let i = 0; i < plotData.length; i++) {
		...
		if (isDefined(yAccessor(d).close)) {
			...
			const priceArray = d.aggTrades;
      if (
        priceArray &&
        priceArray.length !== 0 &&
        (priceArray[priceArray.length - 1].sumArrayStackImbalanceSell.length !==
          0 ||
          priceArray[priceArray.length - 1].sumArrayStackImbalanceBuy.length !==
            0)
      ) {
        // if(priceArray[priceArray.length-1].sumArrayStackImbalanceSell || priceArray[priceArray.length-1].sumArrayStackImbalanceBuy) {

        const { sumArrayStackImbalanceSell, sumArrayStackImbalanceBuy } =
          priceArray[priceArray.length - 1];
        if (sumArrayStackImbalanceSell.length !== 0) {
          for (const num of sumArrayStackImbalanceSell) {
            candlesChild.push({
              // type: "line"
              x: x - offset,
              y: y,
              stackImbalance: {
                ...
                h:
                  Math.round(yScale(num[0])) -
                  Math.round(yScale(num[num.length - 1])),
        ...
        if (sumArrayStackImbalanceBuy.length !== 0) {
          for (const num of sumArrayStackImbalanceBuy) {
            candlesChild.push({
              // type: "line"
              x: x - offset,
              y: y,
...
export default CandlestickSeriesStackImbalance;
