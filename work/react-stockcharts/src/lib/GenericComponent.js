...
import { CANVAS_IDS } from "./CanvasContainer";
...
class GenericComponent extends Component {
	...
	evaluateType(type, e) {
		...
		const proceed = this.props.drawOn.indexOf(newType) > -1 || this.props.eventOn?.indexOf(newType) > -1;
		...
		switch (type) {
			...
			case "mousemove": {
				...
				const { amIOnTop, amIOnTopHovering, setCursorClass, redrawCanvas } = this.context;

				const hovering = this.isHover(e);
				if (!this.props.enablePropagationHover && hovering) {
					this.moreProps.hovering = amIOnTopHovering(this.suscriberId);
				} else {
					this.moreProps.hovering = hovering;
				}

				if (this.moreProps.hovering && this.props.showCursorWhenHovering) {
				...
				if (this.moreProps.hovering && !prevHover || prevHover && !this.moreProps.hovering) {
					redrawCanvas(CANVAS_IDS.BACKGROUND);
				}
			...
			case "drag": {
				if (this.dragInProgress && this.props.onDrag && this.getPanConditions().draggable) {
	...
	getPanConditions() {
		const draggable = !this.props.disableDrag && (
	...
	UNSAFE_componentWillMount() {
		...
		const { groupId, clip, edgeClip } = this.props;

		subscribe(this.suscriberId,
			{
				chartId, groupId, clip, edgeClip,
				enablePropagationHover: this.props.enablePropagationHover,
	...
	componentDidUpdate(prevProps) {
		const { chartCanvasType, updateGroupId } = this.context;
		const { canvasDraw, selected, interactiveCursorClass, groupId } = this.props;
		...
		if (groupId) {
			updateGroupId(this.suscriberId, groupId);
		}
	}
	...
	drawOnCanvas(canvasId) {
		...
		if (!ctx || canvasId && ctx.canvas.id !== canvasId) {
			// no context to draw
			return;
		}
...
GenericComponent.propTypes = {
	...
	showCursorWhenHovering: PropTypes.bool,
	disablePan: PropTypes.bool.isRequired,
	disableDrag: PropTypes.bool,
	enablePropagationHover: PropTypes.bool,
...
GenericComponent.contextTypes = {
	...
	amIOnTopHovering: PropTypes.func.isRequired,
	subscribe: PropTypes.func.isRequired,
	unsubscribe: PropTypes.func.isRequired,
	updateGroupId: PropTypes.func.isRequired,
	setCursorClass: PropTypes.func.isRequired,
	redrawCanvas: PropTypes.func.isRequired,
};
...
export function getBackgroundCanvas(contexts) {
	return contexts?.bg;
}

export function getDrawCanvas(contexts) {
	return contexts?.draw;
}

export function getMouseMarkerCanvas(contexts) {
	return contexts?.mouseMarker;
}
