...
import { deleteBotTrading, getListBots, normalizeBot } from './fetcher';
...
interface Props {
  ...
  handleDelete: (roleId: Bot['id']) => void;
  handleCloneBot: (roleId: Bot['id']) => void;
}
              ...
              data.map((row) => {
                return (
                  <TableBotRow
                    key={row.id}
                    bot={row}
                    handleCloneBot={() => handleCloneBot(row.id)}
...
const PackageTimeListing = (): JSX.Element => {
  ...
  const handleCreate = () => {
    router.push('/bot/tbot/create');
  };

  const handleEditing = (id: Bot['id']) => {
    router.push(`/bot/tbot/${id}`);
  };
  const handleCloneBot = (id: Bot['id']) => {
    router.push(`/bot/tbot/${id}/clone`);
  };
  ...
  const handleDeleteBotTrading = () => {
    if (!roleSelected) return;
    setLoading(true);
    deleteBotTrading(roleSelected)
      .then((res) => {
  ...
  return (
    <>
      <Flex align="center" justifyContent="end" mb="20px">
        {ability.can('ROLE', ability.permissions.CREATE_BOT_TRADING) && (
          ...
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <Button
                isLoading={loading}
                colorScheme="red"
                onClick={handleDeleteBotTrading}
                ml={3}
              >
                Delete
              </Button>
